...
from priml.baselines.speedrundit.data import center_crop_dhariwal
    ...
    *,
    ...
    for index, record in enumerate(labels(iter(image_source))):
        ...
        with Image.open(file_path) as opened:
            cropped = center_crop_dhariwal(
                np.asarray(opened.convert("RGB")), resolution
            )
        Image.fromarray(cropped).save(image_path)
        image = torch.from_numpy(cropped.copy()).permute(2, 0, 1)[None].to(device)
